...
declare function sendChat(as:string, msg:string, callback?:any, options?:object) : void;
declare function getObj(type:string, id:string) : Roll20Object;
declare function log(msg:string) : void;
declare function Campaign() : Roll20Object;
declare function filterObjs(func:(item: Roll20Object) => boolean) : Roll20Object[] ;
declare function playerIsGM(id:string) : boolean;
declare function randomInteger(MAX:number) : number;
declare function getAttrByName(char:string, attr:string, type?:string) : string | number | undefined;
declare function findObjs(attrs:object, options?:object) : Roll20Object[];
declare function on(event:string, func:any) : void;
declare var state : { PF2 : StateObject };
...
interface Command {
    ...
    noTarget?: boolean;
    do(params: any, targets: Roll20Object[], tags:string[], active:boolean) : string;
}

interface Roll20MessageCallback {
    who: string;
    playerid: string;
    type: string;
    content: string;
    origRoll?: string;
    inlinerolls?: Roll20InlineRoll[];
    rolltemplate?: string[];
    target?: string;
    target_name?: string;
    selected?: Roll20Object[];
}

interface Roll20InlineRoll {
    expression: string;
    results: Roll20Roll;
    rollid: string;
    signature: string;
}
interface Roll20Roll {
    type?: string;
    rolls?: Roll20Roll[];
    dice?: number;
    sides?: number;
    results?: Roll20RollResult[];
    mods?: any;
    resultType?: string;
    total?: number;
    table?: string;
}
interface Roll20RollResult {
    v?: string;
    tableItem?: { avatar: string, id: string, name: string, weight: number };
    tableidx?: number;
}
...
var Pathfinder2Utils : any = Pathfinder2Utils || (function() {

    const fieldType: Record<string,number> = {
        stat: 1,
        calculated: 2,
        skill: 3,
        save: 4
    ...
    const statId: Record<string, number> = {
        str: 1,
        dex: 2,
        con: 3,
        int: 4,
    ...
    const st_names: string[] = ["","strength","dexterity","constitution","intelligence","wisdom","charisma"];

    let level_dcs: number[] = [14, 15, 16, 18, 19, 20, 22, 23, 24, 26, 27, 28, 30, 31, 32, 34, 35, 36, 38, 39, 40, 42, 44, 46, 48, 50, 99999];
    ...
    /**
     * Inserts a key-value pair into a dictionary in place. If the key is already present, add
     * distinguishing numbers to the elements to break the tie.
     * @param hash The dictionary.
     * @param key The key to insert.
     * @param value The value to insert.
     */
    function insertDictDeDupe<A>(hash: Record<string, A>, key: string, value: A): void {
        if (hash[key + " 1"]) {
            let index = 1;
     ...
     * @param letter The skill letter.
     * @returns The ordinal value 0-4.
     */
    function skillOrdinal(letter: string): number {
        switch(letter) {
            case "": return 0;
            case "U": return 0;
            case "T": return 1;
     ...
     * @param letter The letter as read from the character sheet.
     * @returns The letter in caps, or U if an invalid value or nothing was specified.
     */
    function standardiseSkillLetter(letter: string): string {
        switch(letter) {
            case "": return "U";
     ...
     */
    function selectedTokens(selected: Roll20Object[]): Roll20Object[] {
        if (selected === undefined) return [];
        // There should be no way a selected entry has a null type or id value.
        let realObjs = selected.map((x) => getObj(x.get("type") as string, x.get("id") as string));
     ...
     * @param dict The dictionary of table items.
     * @returns The Roll20 table string specification.
     */
    function dictToTemplate(title: string, dict : Record<string, string|number>): string {
     ...
     * @param token The graphic object.
     * @returns The player object or null.
     */
    function getCharForToken(token: Roll20Object) : (Roll20Object | null) {
        if (!token.get) {
        ...
        if ((charId === "") || (charId === undefined)) return null;
        return getObj("character", charId as string);
    }

    /**
     * Get the name for a token, from its character or itself, or else an unknown placeholder.
     * @param token The graphic object.
     * @return The associated character's name, or the token's name, or a combination if they don't match, or "(Unknown)".
     */
    function getTokenName(token: Roll20Object): string {
        let char = getCharForToken(token);
        if (char !== null) {
            let charName = char.get("name") as string|undefined;
            if (charName) {
                let tokenName = token.get("name") as string|undefined;
                if ((tokenName) && (abbreviate(tokenName) !== abbreviate(charName))) {
                    return tokenName + " [" + charName + "]";
                } else {
                    return charName;
                }
            }
        }
        if ((token.get("name"))) return token.get("name") as string;
        return "(Unknown)";
    }

    /**
     * Tests if a token's name matches a specification.
     * @param token The graphic object.
     * @param specName The specification.
     * @returns TRUE if the character's name, or token's name, starts with the canonization of the specName.
     */
    function tokenNameMatches(token: Roll20Object, specName: string): boolean {
     ...
     * @returns Array of all token objects on the page.
     */
    function getPageTokens() : Roll20Object[] {
     ...
     * @param token The graphic object.
     * @returns True if the token is a character, has controllers, and at least one of them is not the GM.
     */
    function tokenIsPC(token: Roll20Object) : boolean {
        let char = getCharForToken(token);
        if (!char) return false;
        let controllers = char.get("controlledby") as string|undefined;
        if (!controllers) return false;
        return (_.some(controllers.split(","), (x => !playerIsGM(x))));
    }

    /**
     * The processInlineRolls function from the script cookbook.
     * @param msg A Roll20 message structure.
     * @returns The content of the message with inline rolls substituted for their results.
     */
    function processInlinerolls(msg: Roll20MessageCallback) : string {
        if(_.has(msg,'inlinerolls')){
            return _.chain(msg.inlinerolls!)
                .reduce(function(m:Record<string,string|number>,v,k){
                    var ti=_.reduce(v.results.rolls!,function(m2:string[],v2){
                        if(_.has(v2,'table')){
                            m2.push(_.reduce(v2.results!,function(m3:string[],v3){
                ...
                .reduce(function(m,v,k){
                    return m.replace(k,v as string);
                },msg.content)
                .value();
        } else {
            return msg.content;
     ...
     * @returns The tokens it likely refers to.
     */
    function findTargetToken(specifier: string) : Roll20Object[] {
     ...
     * @returns The parsed turn order.
     */
    function getTurnOrder(): { id : string, pr : string, custom : string }[] {
        let strOrder = Campaign().get("turnorder") as string|undefined;
     ...
     * @param newOrder The new turn order value.
     */
    function updateTurnOrder(token: Roll20Object, newOrder: number): void {
        let order = getTurnOrder();
        order = _.reject(order, x => x.id === token.id);
        let pos;
        for (pos=0; pos<order.length; pos++) {
            if (parseInt(order[pos].pr,10) < newOrder) break;
        }
        order.splice(pos, 0, {"id": token.id, "pr": newOrder.toString(), "custom": ""});
        Campaign().set("turnorder", JSON.stringify(order));
    }

    /**
     * Rolls a d20 using the approved RNG.
     * @returns A random number from 1-20.
     */
    function d20(): number {
     ...
     * @param max True if the maximum value should be retured.
     * @returns The value, in whatever type Roll20 has it, or undefined if the attribute is missing or the token has no character.
     */
    function getTokenAttr(token: Roll20Object, property: string, max: boolean = false) : string | number | undefined {
        let char = getCharForToken(token);
    ...
    /**
     * Set the named attribute value of the character represented by given token.
     * Logs a warning if the token doesn't have a related character.
     * @param token The Graphic object.
     * @param property The value to set.
     * @param value The value to set it to.
     */
    function setTokenAttr(token: Roll20Object, property: string, value: string|number): void {
        let char = getCharForToken(token);
        if (!char) {
            log("Failed to find/set attribute " + property + " on token " + token.id + " because no linked character was found.");
            return;
        }
     ...
     * @param strname The internal field name.
     * @returns The data stored about the field, or undefined if it doesn't exist.
     */
    function getNamedField(strname: string): Field | undefined {
     ...
     * @param strname The internal field name.
     * @returns The Roll20 attribute name.
     */
    function namedFieldToAttrName(strname: string): string {
        let field = getNamedField(strname);
        if (field === undefined) {
            log("Bad field requested " + strname);
        }
        return field!.field;
    }
     ...
     * @returns The new string sum.
     */
    function appendNumToSum(sum: string, number: number): string {
     ...
     * @param roll The rolled value.
     * @returns The highest standard DC the roll can beat.
     */
    function highestLevelRollBeats(roll: number): number {
        let level = 0;
     ...
     * @param target The target carrying the modifiers.
     * @param attributes The list of attributes to roll (and sum).
     * @param modifiers Additional number modifiers to add.
     * @param tags List of extra (non-implicit) tags to apply.
     * @returns The roll result as a RollSpec object.
     */
    function rollAttribute(target: Roll20Object, attributes: Field[], modifiers: number[], tags: string[]) : RollSpec {
        let impliedtags = [];
        let modString = "";
        let ok = false;
        let modtotal = 0;
        ...
        for (let attr of attributes) {
            ...
            if (propertyValue == null) {
                ...
                let numProp = parseInt(propertyValue as string,10);
                if (isNaN(numProp)) {
                    numProp = 0;
                    bad = true;
                }
     ...
     * By definition assurance ignores all modifiers, so they cannot be specified.
     * @param target The target.
     * @param skill The skill name (must be a skill)
     * @returns The result (as a roll, even though it's not really a roll)
     */
    function skillAssurance(target: Roll20Object, skill: string): RollSpec  {
        let profBonus = getTokenAttr(target, skill+"_proficiency");
        let profLetter = getTokenAttr(target, skill+"_proficiency_display");
        if (profBonus === undefined || profBonus === null) {
            return { "text": "(missing)", "roll": 10 };
        }
        let profInt = parseInt(profBonus as string,10);
        if (isNaN(profInt)) {
     ...
     * @param abilityString The user specification of the ability.
     * @param freeSkill The user specification of the skill to be used for free skill abilities.
     * @param targets The list of targets.
     * @param tags Additional tags for the roll.
     * @returns The command response.
     */
    function doAbility(abilityString: string, freeSkill: string|undefined, targets: Roll20Object[], tags: string[]): string {
        ...
        for (let target of targets) {
            ...
            let skillLevel = getTokenAttr(target, skill + "_proficiency_display") as string|undefined;
            if (!skillLevel) {
            ...
            // All ok, do the roll
            if (!getNamedField(skill)) {
                results[name] = "(Skill entry is broken!)";
                continue;
            }
        ...
        let infodict: Record<string, string> = {
     ...
     * @param property The user specification of the property name.
     * @param targets The target list.
     * @param tags Additional tags for modifiers.
     * @returns The command result.
     */
    function getProperty(property: string, targets: Roll20Object[], tags: string[]) : string {
        let results: Record<string, string> = {};
        ...
        let fulltags = tags.concat(impliedtags);
        for (let target of targets) {
            let propertyValue = getTokenAttr(target, attr.field) as string|undefined;
            let name = getTokenName(target);
            let tagmod = calculateTotalMod(target, fulltags);
     ...
     * @param property The property name to roll.
     * @param targets The target list.
     * @param isInit Should initiative modifier be added?
     * @param setInit Should turn tracker be updated?
     * @param tags List of extra rolltags.
     * @returns Command response.
     */
    function rollProperty(property: string, targets: Roll20Object[], isInit: boolean, setInit: boolean, tags: string[]): string {
        let results: Record<string, string> = {};
        let attr = getNamedField(property);
        if (attr === undefined) {
            return ("Unknown character property, " + property);
     ...
     * @param property The property name.
     * @param targets The target list.
     * @param tags Tags for calculating modifiers.
     * @returns Command response.
     */
    function bestProperty(property: string, targets: Roll20Object[], tags: string[]): string {
        let bestName = "";
        let bestValue = -9999;
        let bestText = "";
        let impliedtags: string[] = [];
        ...
        for (let target of targets) {
            let tagmod = calculateTotalMod(target, fulltags);
            let value = getTokenAttr(target, attr.field) as string|undefined;
            if (!value) continue;
            ...
            if (effValue > bestValue) {
                ...
                if (tagmod == 0) {
                    bestText = effValue.toString();
                } else {
                    bestText = appendNumToSum(value, tagmod) + " = " + effValue.toString();
                }
            }
        }
        if (bestValue === -9999) {
     ...
     * the "assure" command.
     * @param property The property name (must be a skill)
     * @param targets The target name.
     * @returns Command output.
     */
    function assureProperty(property: string, targets: Roll20Object[]): string {
        let results = {};
        let attr = getNamedField(property);
        if (attr === undefined) {
            return ("Unknown character property, " + property);
     ...
     * @param spec The target specifier, including the initial @.
     * @returns The list of targets found (which may be empty)
     */
    function getSpecifiedTargets(spec: string): Roll20Object[] {
        let targets: Roll20Object[] = [];
     ...
     * @returns The targetted objects.
     */
    function getInferredTargets(msg: Roll20MessageCallback): Roll20Object[] {
        ...
        for (let token of allTokens) {
            let char = getCharForToken(token);
            if (!char) continue;
            let controller = char.get("controlledby") as string|undefined;
            if (!controller) continue;
            if (_.some(controller.split(","),(x => x === msg.playerid))) {
                possTokens.push(token);
            }
        }
        return possTokens;
    }
     ...
     * @returns Command output.
     */
    function listMods(tags: string[]): string {
     ...
     * @param cat The modifier category.
     * @param amount The value of the modifier.
     * @param targets The targeted tokens.
     * @param tags The affected rolltags.
     * @returns Command output.
     */
    function addMod(name:string|undefined,
                    cat:string|undefined,
                    amount:number|undefined,
                    targets:Roll20Object[],
                    tags:string[]): string {
     ...
     * @returns Command output.
     */
    function delMod(name: string): string {
        if (name === undefined) return "Modifier name missing.";
        let existingIndex = state.PF2.modifiers.findIndex(x => x.name === name);
        if (existingIndex === -1) {
     ...
     * @param tags The list of rolltags.
     * @returns Does the modifier apply?
     */
    function modApplies(mod: Modifier, target: Roll20Object, tags: string[]): boolean {
     ...
     */
    function calculateTotalMod(target: Roll20Object, tags: string[]) {
        let bests: Record<string, number> = {};
        let worsts: Record<string, number> = {};
        let total = 0;
    ...
    function damage(amount: number, targets: Roll20Object[], tags: string[], active: boolean) {
        ...
        for (let target of targets) {
            ...
            if (char !== null) {
                ...
                amount = amount + mod;
                if (tokenIsPC(target)) {
                    let hp = getTokenAttr(target, "hit_points") as number|undefined;
                    if (hp !== undefined) {
                        setTokenAttr(target, "hit_points", (hp-amount));
                        insertDictDeDupe(results, getTokenName(target), (hp-amount).toString());
                    } else {
                        insertDictDeDupe(results, getTokenName(target), "[Cannot apply damage because no HP value on PC sheet.]");
                    }
                } else {
                    if (!target.get("bar1_max")) {
                        let hp = getTokenAttr(target, "hit_points") as number|undefined;
                        if (hp !== undefined) { // Can't use if hp because it might be zero..
                            target.set("bar1_max", getTokenAttr(target, "hit_points", true) as number);
                            target.set("bar1_value", hp - amount);
                            insertDictDeDupe(results, getTokenName(target), hp - amount);
                        } else {
                            insertDictDeDupe(results, getTokenName(target), "[Cannot apply damage because no HP value found on sheet or bars.");
                        }
                    } else {
                        let hpstr = target.get("bar1_value") as string|undefined;
                        if (!hpstr) {
                            insertDictDeDupe(results, getTokenName(target), "[Cannot apply damage because no HP value found on sheet or bars.]");
                        } else {
    ...
    function heal(amount: number, targets: Roll20Object[], tags: string[], active: boolean) {
        ...
        for (let target of targets) {
            ...
            if (char !== null) {
                ...
                if (tokenIsPC(target)) {
                    let hp = getTokenAttr(target, "hit_points") as number|undefined;
                    let maxHp = getTokenAttr(target, "hit_points", true) as number|undefined;
                    if (hp !== undefined) {
                        let result:number;
                        if (maxHp !== undefined)
                            result = Math.min(hp+amount, maxHp);
                        else {
                            result = hp+amount;
                        }
                        setTokenAttr(target, "hit_points", result);
                        insertDictDeDupe(results, getTokenName(target), result.toString());
                    } else {
                        insertDictDeDupe(results, getTokenName(target), "[Cannot apply healing because no HP value on PC sheet.]");
                    }
                } else {
                    if (!target.get("bar1_max")) {
                        let hp = getTokenAttr(target, "hit_points") as number|undefined;
                        ...
                        if (hp !== undefined) {
                            ...
                            insertDictDeDupe(results, getTokenName(target), "[Can't apply healing because no HP value found on bars or sheet.]");
                        }
                    } else {
                        let hpStr = target.get("bar1_value") as string|undefined;
                        let maxHpStr = target.get("bar1_max") as string|undefined;
                        if (!hpStr) {
                            insertDictDeDupe(results, getTokenName(target), "[Can't apply healing because no HP value found on bars or sheet.]");
                        } else {
                            let hp = parseInt(hpStr,10);
                            if (!isNaN(hp)) {
                                ...
                                insertDictDeDupe(results, getTokenName(target), "[Can't apply healing because bar1 value is non-numeric.]");
                            }
                        }
                    }
                }
    ...
    function explainMods(targets: Roll20Object[], tags: string[]): string {
        let out: string = "";
        let bests: Record<string, Modifier> = {};
        let worsts: Record<string, Modifier> = {};
        for (let target of targets) {
            for (let mod of state.PF2.modifiers) {
                if (modApplies(mod, target, tags)) {
                    if (mod.cat !== "u") {
                        ...
                        if (imod >= 0) {
                            let oldBest = bests[mod.cat];
                            if ((oldBest === undefined) || (oldBest.value <= imod)) {
                                bests[mod.cat] = mod;
                            }
                        } else {
                            let oldWorst = worsts[mod.cat];
                            if ((oldWorst === undefined) || (oldWorst.value >= imod)) {
                                worsts[mod.cat] = mod;
                            }
                        }
                    }
     ...
     */
    function message(msg: Roll20MessageCallback) {
        // Check it's for us.
        if (msg.type === "api") {
            if (msg.content.startsWith("!pf")) {
                ...
                if (secret) {
                    ...
                    if (response !== undefined) {
                        ...
                        log("Command returned an undefined response.");
                    }
                }
            }
        }
     ...
     */
    function tokenGone(obj: Roll20Object): void {
    ...
    function install(): void {
        on("chat:message", (msg: Roll20MessageCallback) => message(msg));
        on("destroy:graphic", (obj: Roll20Object) => tokenGone(obj));
